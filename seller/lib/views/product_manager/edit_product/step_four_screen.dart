...
import 'package:seller/views/product_manager/edit_product/step_five_screen.dart';
...
class StepFourScreen extends StatefulWidget {
  ...
  const StepFourScreen({super.key, required this.formData, required this.product});
...
class _StepFourScreenState extends State<StepFourScreen> {
  ...
  void initState() {
    ...
    weightUnits = [];
    donationTypes = [];
    selectedPackageType = widget.product['package_type'];
    var dimensions = widget.product['package_dimensions'].toString().split(",");
    lengthController = TextEditingController(text: dimensions[0].toString());
    widthController = TextEditingController(text: dimensions[1].toString());
    heightController = TextEditingController(text: dimensions[2].toString());
    weightController = TextEditingController(text: dimensions[3].toString());
    var dimUnits = widget.product['package_units'].toString().split(",");
    selectedDimensionUnit = dimUnits[0].toString();
    selectedWeightUnit = dimUnits[1].toString();
    _db = Database();
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        buildActionButton(
                          label: "Next",
                          onPressed: () async {
                            final res = await Navigator.push(context, MaterialPageRoute(builder: (context) => StepFiveScreen(
